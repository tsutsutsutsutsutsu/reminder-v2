...
# .env読み込み（Railwayでは無視されるけどローカル用に）
load_dotenv()
...
def handle_message(event):
    ...
    if text.startswith("リマインド "):
        try:
            ...
            new_row = [f"{datetime.now()}", reminder_text, remind_dt.strftime("%Y-%m-%d %H:%M"), user_id, ""]
            ...
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text="登録失敗しました。フォーマット: リマインド YYYY-MM-DD HH:MM メッセージ")
            )
    else:
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text="リマインドを設定するには「リマインド 日時 メッセージ」と送ってね！")
        )
